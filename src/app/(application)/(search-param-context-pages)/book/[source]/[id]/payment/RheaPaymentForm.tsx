...
import {
  ActionButton,
  FormikCheckBox,
} from "~/app/ui/components/authentication";
...
export default function RheaPaymentForm({
  ...
  useEffect(() => {
    if (paymentCompleted && router) {
      const queryString =
        searchParams instanceof URLSearchParams ? searchParams.toString() : "";
      router.replace(
        `/book/${params.source}/${params.id}/reservation-successful?${queryString}`,
      );
    }
